  ...
  return (
    <Box component="section" sx={{ py: { xs: 3.5, md: 6 }, bgcolor: '#faf9f6' }}>
      <Box sx={{ maxWidth: 1140, mx: 'auto', px: { xs: 2, sm: 3 } }}>
        <Grid container columnSpacing={4.4} rowSpacing={3.5} alignItems="start">
                  ...
                  <Box sx={{
                    ...
                    <Box component="ul" sx={{ listStyle: 'none', m: 0, p: 0, display: 'grid', gap: 0.7 }}>
                      {['Carrera 25 # 42 a 28', 'Cali, Valle del Cauca', 'Comunas 12, 1, 4 y 17'].map((item) => (
                        <Box component="li" key={item} sx={{ position: 'relative', pl: 1.6, color: 'text.secondary', fontSize: '0.96rem', '&::before': { content: '""', position: 'absolute', left: 0, top: '0.62em', width: '0.5rem', height: '0.5rem', borderRadius: '50%', bgcolor: '#ffd200' } }}>
                          {item}
                        </Box>
                      ))}
                    </Box>
